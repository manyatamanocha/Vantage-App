...
A real, working web app — not a clickable prototype or a case-study-only artifact. Auth, backend, real Claude API calls for the recommendation engine and Handback artifact, and real persistence.
...
- **AI**: Claude API (Anthropic SDK), called **server-side only** (route handlers/server actions) — the API key never reaches the browser.
...
2. **Structure** — one Claude call extracts `goal` + `problem_type` from the raw input → returned as editable fields (Edit affordance) before continuing. Every AI output the user sees is confirmable/correctable, nothing is shown as final without a chance to correct it.
...
4. **Reveal** — one Claude call, given the structured problem + the user's guess, returns match/mismatch, why-it-fits, why-alternatives-don't (comparative reasoning), and tool class. Saved to the `solves` row.
5. **Handback** — separate, on-demand Claude call from the session summary screen, triggered only if the user requests the artifact.

## Error handling

Each Claude call site (structure, reveal, handback) is wrapped with a single retry + timeout. Input already persisted in an earlier step is never lost on a later step's failure — only the AI-dependent step shows a retry state. No fallback content is invented on failure.

## Testing

- **Unit**: prompt-building and response-parsing for each of the three Claude call sites — mock the Anthropic client, assert correct extraction/shape.
...
- Persona vs. wireframe conflict: `Persona.md` says "mainly consulting" (firm-employed); the wireframe Settings screen labels the persona "independent consultant." Unresolved — pick one before the Settings/persona copy is implemented in Phase covering that screen.
