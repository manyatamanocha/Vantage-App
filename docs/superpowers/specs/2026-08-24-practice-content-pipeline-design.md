...
- **A running pipeline can reshuffle a user's in-progress practice day.** `getTodaysPracticeCase()` (`app/practice/today/actions.ts`) fetches the active pool ordered by `id` ascending and picks a fixed index via `pickDailyIndex(dailySeed(...), pool.length)` — deterministic only as long as both `pool.length` and each row's ordinal position stay fixed for that day. Inserting new rows changes `pool.length` and, because `practice_cases.id` is a random `gen_random_uuid()`, scatters new rows into arbitrary positions in that ordering rather than appending at the end — so a pipeline run between two reloads of `/practice/today` can hand the same user a different case mid-day. Flagged during this plan's final review; not a bug in this plan (no scheduler exists yet, so it cannot trigger today) but a hard constraint on whoever wires one. Two ways out, neither implemented here: (a) only ever run the pipeline at a fixed off-peak boundary (e.g. just after UTC midnight, before any day's picks are made), or (b) make the daily pick itself insertion-stable — e.g. add a monotonic `created_at`/sequence column and order/pick against a snapshot bounded by it, instead of ordering by `id`. Either requires a decision + a small schema or query change beyond this plan's scope.
