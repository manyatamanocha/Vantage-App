...
- **Roadmap: surface the rest of the analytics stack on `/admin/analytics`.** `lib/analytics/metrics.ts` already computes and unit-tests the full metric set — D2 return rate, practice start rate (7-day return), D30/4-week habit retention, active/inactive/dormant engagement split, per-surface breakdown, and solution feedback — but only the North Star (Weekly Engaged Learners), activation rate, practice completion, and D7 retention are actually rendered. Deliberately held back (2026-08-26 session, direct user decision): with only ~94 real events so far (mostly login/signup, one `solve_started`, zero `meaningful_activity_completed`), the retention-family metrics (D2/D7/D30, habit retention, practice start rate) would show a permanently-empty "—" cohort, which reads as broken rather than pending. Revisit once there's a real `meaningful_activity_completed` cohort to measure — nothing needs rebuilding, just wiring the existing functions into `app/admin/analytics/page.tsx`.
