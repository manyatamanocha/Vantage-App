...
export function ScenarioQuizSession() {
  ...
  // to guarantee no repeats, so it can't also drive a "2 / 90" progress
  ...
  function nextQuestion() {
    // Drop the scenario just shown instead of wrapping the index around it —
    // a scenario must never repeat within a sitting either, only once per pool.
    if (questions) {
    ...
    return (
      <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col px-5 py-8 sm:px-8 sm:py-12">
        <div className="topline">
          <span className="datechip">Scenario Quiz</span>
        </div>
        <header>
